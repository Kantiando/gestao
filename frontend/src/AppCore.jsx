import { useEffect, useMemo, useState } from "react";
import { fluxoVazio, optionalRequest } from "./fallbacks.js";

const competencia = "2025-06";
const dataInicio = "2025-06-01";
const dataFim = "2025-06-30";

function apiBaseUrl() {
  if (window.location.hostname.includes("onrender.com")) return "https://gestao-api.onrender.com";
  return "http://localhost:8000";
}

async function request(path) {
  const response = await fetch(`${apiBaseUrl()}${path}`);
  if (!response.ok) throw new Error(`API ${response.status}: ${path}`);
  return response.json();
}

function money(value) {
  return new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(value || 0);
}

function percent(value) {
  return `${Number(value || 0).toFixed(2)}%`;
}

export default function AppCore() {
  const [view, setView] = useState("geral");
  const [empresas, setEmpresas] = useState([]);
  const [empresaId, setEmpresaId] = useState("");
  const [dreGeral, setDreGeral] = useState(null);
  const [fluxoGeral, setFluxoGeral] = useState(fluxoVazio);
  const [dashboard, setDashboard] = useState(null);
  const [contas, setContas] = useState([]);
  const [lancamentos, setLancamentos] = useState([]);
  const [erro, setErro] = useState("");

  const empresaAtual = useMemo(() => empresas.find((item) => item.id === empresaId), [empresas, empresaId]);

  useEffect(() => {
    async function carregarGeral() {
      try {
        const empresasData = await request("/empresas");
        const dreData = await request(`/dre/consolidado?competencia=${competencia}`);
        const fluxoData = await optionalRequest(request, `/fluxo-caixa/consolidado?data_inicio=${dataInicio}&data_fim=${dataFim}`, fluxoVazio);
        setEmpresas(empresasData);
        setDreGeral(dreData);
        setFluxoGeral(fluxoData);
        if (empresasData[0]) setEmpresaId(empresasData[0].id);
        setErro("");
      } catch (error) {
        setErro(`Não foi possível carregar os dados principais da API. ${error.message}`);
      }
    }
    carregarGeral();
  }, []);

  useEffect(() => {
    async function carregarEmpresa() {
      if (!empresaId) return;
      try {
        const dashData = await request(`/empresas/${empresaId}/dashboard?competencia=${competencia}`);
        const contasData = await request(`/empresas/${empresaId}/plano-contas`);
        const lancamentosData = await request(`/empresas/${empresaId}/lancamentos?competencia=${competencia}`);
        setDashboard({ ...dashData, fluxo_caixa: dashData.fluxo_caixa || fluxoVazio });
        setContas(contasData);
        setLancamentos(lancamentosData);
        setErro("");
      } catch (error) {
        setErro(`Não foi possível carregar a empresa. ${error.message}`);
      }
    }
    carregarEmpresa();
  }, [empresaId]);

  const dreEmpresa = dashboard?.dre;
  const fluxoEmpresa = dashboard?.fluxo_caixa || fluxoVazio;

  return (
    <div className="app">
      <aside className="sidebar">
        <div className="brand"><div className="logo">G</div><div><strong>Gestão</strong><span>Holding privada</span></div></div>
        <button className={view === "geral" ? "active" : ""} onClick={() => setView("geral")}>Visão Geral</button>
        <button className={view === "empresa" ? "active" : ""} onClick={() => setView("empresa")}>Empresa</button>
        <button className={view === "fluxo" ? "active" : ""} onClick={() => setView("fluxo")}>Fluxo de Caixa</button>
        <button className={view === "dre" ? "active" : ""} onClick={() => setView("dre")}>DRE</button>
        <button className={view === "plano" ? "active" : ""} onClick={() => setView("plano")}>Plano de Contas</button>
        <button className={view === "metas" ? "active" : ""} onClick={() => setView("metas")}>Metas</button>
        <label>Empresa ativa</label>
        <select value={empresaId} onChange={(e) => setEmpresaId(e.target.value)}>
          {empresas.map((empresa) => <option key={empresa.id} value={empresa.id}>{empresa.apelido || empresa.nome}</option>)}
        </select>
      </aside>

      <main>
        <header><span>Competência {competencia} · Caixa {dataInicio} até {dataFim}</span><h1>{titulo(view, empresaAtual)}</h1><p>Core inicial de gestão empresarial.</p></header>
        {erro ? <div className="erro">{erro}</div> : null}
        {view === "geral" && <Resumo dre={dreGeral} fluxo={fluxoGeral} empresas={empresas} />}
        {view === "empresa" && <Empresa dre={dreEmpresa} fluxo={fluxoEmpresa} lancamentos={lancamentos} />}
        {view === "fluxo" && <Fluxo fluxo={fluxoEmpresa} />}
        {view === "dre" && <Dre dre={dreEmpresa} />}
        {view === "plano" && <Plano contas={contas} />}
        {view === "metas" && <Metas metas={dashboard?.metas || []} />}
      </main>
    </div>
  );
}

function titulo(view, empresa) {
  const nome = empresa?.apelido || empresa?.nome || "Empresa";
  return { geral: "Visão geral da holding", empresa: `${nome} - Dashboard`, fluxo: `${nome} - Fluxo de caixa`, dre: `${nome} - DRE gerencial`, plano: `${nome} - Plano de contas`, metas: `${nome} - Metas` }[view];
}

function Card({ label, value, hint }) {
  return <article className="card"><span>{label}</span><strong>{value}</strong>{hint && <small>{hint}</small>}</article>;
}

function Resumo({ dre, fluxo, empresas }) {
  return <section className="grid"><Card label="Receita total" value={money(dre?.receita_bruta)} /><Card label="Entradas caixa" value={money(fluxo?.entradas)} /><Card label="Saídas caixa" value={money(fluxo?.saidas)} /><Card label="Saldo caixa" value={money(fluxo?.saldo_periodo)} /><Card label="Lucro líquido" value={money(dre?.lucro_liquido)} /><Card label="Margem líquida" value={percent(dre?.margem_liquida_percentual)} /><div className="panel"><h2>Empresas</h2>{empresas.map((empresa) => <p key={empresa.id}>{empresa.nome}</p>)}</div></section>;
}

function Empresa({ dre, fluxo, lancamentos }) {
  return <section className="grid"><Card label="Receita DRE" value={money(dre?.receita_bruta)} /><Card label="Entradas caixa" value={money(fluxo?.entradas)} /><Card label="Saídas caixa" value={money(fluxo?.saidas)} /><Card label="Saldo caixa" value={money(fluxo?.saldo_periodo)} /><Card label="Lucro líquido" value={money(dre?.lucro_liquido)} /><TabelaLancamentos lancamentos={lancamentos} /></section>;
}

function Fluxo({ fluxo }) {
  return <section className="grid"><Card label="Entradas" value={money(fluxo?.entradas)} /><Card label="Saídas" value={money(fluxo?.saidas)} /><Card label="Saldo" value={money(fluxo?.saldo_periodo)} /><Card label="Movimentos" value={fluxo?.movimentos?.length || 0} /><div className="panel"><h2>Movimentações de caixa/banco</h2><table><thead><tr><th>Data</th><th>Descrição</th><th>Categoria</th><th>Tipo</th><th>Valor</th></tr></thead><tbody>{(fluxo?.movimentos || []).map((item) => <tr key={item.id}><td>{item.data_movimento}</td><td>{item.descricao}</td><td>{item.categoria}</td><td>{item.tipo}</td><td>{money(item.valor)}</td></tr>)}</tbody></table></div></section>;
}

function Dre({ dre }) {
  const linhas = [["Receita Bruta", dre?.receita_bruta], ["Custos Variáveis", dre?.custos_variaveis], ["Lucro Bruto", dre?.lucro_bruto], ["Despesas Operacionais", dre?.despesas_operacionais], ["Resultado Operacional", dre?.resultado_operacional], ["Lucro Líquido", dre?.lucro_liquido]];
  return <div className="panel"><h2>DRE simplificada</h2><table><tbody>{linhas.map(([label, value]) => <tr key={label}><td>{label}</td><td>{money(value)}</td></tr>)}</tbody></table></div>;
}

function Plano({ contas }) {
  return <div className="panel"><h2>Plano de contas</h2><table><thead><tr><th>Código</th><th>Nome</th><th>Tipo</th><th>Grupo</th></tr></thead><tbody>{contas.map((conta) => <tr key={conta.id}><td>{conta.codigo}</td><td>{conta.nome}</td><td>{conta.tipo}</td><td>{conta.grupo}</td></tr>)}</tbody></table></div>;
}

function Metas({ metas }) {
  return <section className="metas">{metas.map((meta) => <article key={meta.id} className="meta"><strong>{meta.nome}</strong><span>Meta: {money(meta.valor_meta)}</span><span>Realizado: {money(meta.realizado)}</span><div className="bar"><div style={{ width: `${Math.min(meta.progresso, 100)}%` }} /></div><small>{percent(meta.progresso)} concluído</small></article>)}</section>;
}

function TabelaLancamentos({ lancamentos }) {
  return <div className="panel"><h2>Lançamentos DRE</h2><table><thead><tr><th>Data</th><th>Descrição</th><th>Tipo</th><th>Valor</th></tr></thead><tbody>{lancamentos.map((item) => <tr key={item.id}><td>{item.data_lancamento}</td><td>{item.descricao}</td><td>{item.tipo}</td><td>{money(item.valor)}</td></tr>)}</tbody></table></div>;
}
