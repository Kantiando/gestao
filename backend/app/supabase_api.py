...
app = FastAPI(title="Gestao Empresarial API", version="0.4.0-supabase")
...
def check_config():
    if not SUPABASE_URL or not SUPABASE_KEY:
        raise HTTPException(status_code=500, detail="SUPABASE_URL e SUPABASE_KEY precisam estar configurados no Render")
...
@app.get("/")
def root(): return {"app": "gestao-api", "version": "0.4.0-supabase"}
@app.get("/health")
def health(): return {"status": "ok"}
@app.get("/version")
def version(): return {"app": "gestao-api", "version": "0.4.0-supabase", "database": "supabase"}
@app.get("/empresas")
